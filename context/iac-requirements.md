...
The todo-service is a full-stack task management application with a React SPA frontend and a Node.js/Express CRUD API. The API uses an intentional in-memory todo store, so the service is stateless and does not require a database for this lab. It will be containerized and hosted on AWS ECS Fargate behind an Application Load Balancer, with CloudWatch logs and Container Insights for observability.
